...
bool Jseb2_EventBuffer::CheckBufferSpace() {
  ...
  //std::cout << __FILE__ << " CheckBufferSpace: remainingbytes = " << remainingbytes << " bufsize " << nbytes << endl;
...
bool Jseb2_EventBuffer::StartEvent() {
  ...
  _nWordsInEvent += 8;
  //cout << __FILE__ << " LINE " << __LINE__ << " write buffer START EVENT " << endl;
  return true;
}
...
bool Jseb2_EventBuffer::EndEvent() {
  ...
  //cout << __FILE__ << " LINE " << __LINE__ << " write END EVENT number " << _event_number << endl;
  return true;
}
