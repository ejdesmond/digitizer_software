...
void jseb2Controller::jseb2_seb_reader(Dcm2_JSEB2 *jseb2, bool printWords, bool printFrames, bool writeFrames)
{
  ...
  bool debugRcdaqEventReadoutCount = false;
		  ...
		  while(pframe < pwords[idma])
		      ...
		      if(nFrameWords - ioffset < DMAprogress)
			{
			  ...
			  if(iframe > nFramesPerEvent)
				...
				cout << __FILE__ << " line " << __LINE__ << "\t EVENT NUMBER = " << rcdaqEventNumber << " ievent = " << ievent << endl;
				...
				pthread_mutex_unlock(&M_cout);
			      ...
			      if (debugRcdaqEventReadoutLock) {
				pthread_mutex_lock(&M_cout);
				cout << __FILE__ << " LINE " << __LINE__ << " set rcdaqdataready FALSE UNLOCK DataReadyReset" << endl;
				pthread_mutex_unlock(&M_cout);
			      }
	  ...
	  if (debugDma) 
	    {
		  ...
		  if(nFrameWords - ioffset < DMAprogress)
		      ...
		      if(iframe == 0)
			{
			  ...
			  if (debugRcdaqEventReadoutLock) {
				pthread_mutex_lock(&M_cout);
				cout << __FILE__ << " LINE " << __LINE__ << " Start New Event set rcdaqdataready FALSE" << endl;
				pthread_mutex_unlock(&M_cout);
			   }
                          ...
                          if (debugEventReadout)
                            {
                              ...
                              cout << "LINE " << __LINE__ << " nFramesPerEvent = " << nFramesPerEvent << endl;
                              cout << "LINE " << __LINE__ << " frameWords[0] = " << hex << frameWords[0] << dec << endl;
                              cout << "LINE " << __LINE__ << " frameWords[1] = " << hex << frameWords[1] << dec << endl;
                              cout << "LINE " << __LINE__ << " frameWords[2] = " << hex << frameWords[2] << dec << endl;
                              cout << "LINE " << __LINE__ << " frameWords[3] = " << hex << frameWords[3] << dec << endl;
                            }
			  ...
			  if(printFrames)
			    {
			      ...
			      cout << "LINE " << __LINE__ << "   PART3 Frame: ";
			  ...
			  if(frameWords[0] != 0x8)
			    {
			      cout << "LINE " << __LINE__ << " Sync frame has incorrect length... frameWords[0] = 0x " << hex << frameWords[0] << dec << endl;
			      exitRequested = true; printWords = true;
			      failDetected = true;
			      break;
			    }
			  ...
			  if(frameWords[0] != 0x8)
			      ...
			      if(writeFrames)
				{
				  ...
				  if (debugRcdaqEventReadoutCount)
				    {
				      pthread_mutex_lock(&M_cout);
				      cout << __FILE__ << " LINE " << __LINE__ << " set file buffer event number  = " << event_number + event_offset<< endl;
				      pthread_mutex_unlock(&M_cout);
				    }
				  if (debugRcdaqEventReadoutCount)
				    {

				      cout << __FILE__ << " LINE " << __LINE__<< " CopyFrameIntoBuffer words " << nFrameWords << " event# " << event_number+event_offset << endl;
				    }
			  ...
			  if(writeFrames)
			    {
			      ...
			      if (debugRcdaqEventReadoutCount)
			    {
			      cout << __FILE__ << " LINE " << __LINE__ << " WROTE END EVENT " << ievent << " requested events = " << nevents << endl;
			    }
			      // RCDAQ+
			      //pendframe = pframe - sizeof(UINT32);
				...
				//cout << "\t EVENT SIZE = " << storedeventsize << endl;
				cout << __FILE__ << " LINE " << __LINE__ << "\t End EVENT NUMBER = " << rcdaqEventNumber << " ievent = " << ievent << endl;
                                cout << __FILE__ << " LINE " << __LINE__ << " storedeventsize = " << storedeventsize << endl;
				}
			      ...
			      for (int itmp = 0; itmp < storedeventsize ; itmp++ )
				{
				  ...
				  if (debugRcdaqEventReadout)
					...
					cout << "\t tbuff["<<itmp<<"]= " << hex << tmpeventbuffer[itmp] << dec << endl;
                              ...
                              if (debugRcdaqEventReadoutCount) {
                                cout << __FILE__ << " LINE " << __LINE__ << " DONE TRANSFERRIND DATA of size storedeventsize = " << storedeventsize << endl;
                                cout << __FILE__ << " LINE " << __LINE__ <<  " Tell rcdaq we have data to read " << endl;
                              }
			      /*
				...
				storedeventpointers[rcdaqEventNumber].eventNumber = rcdaqEventNumber;
			      ...
			      */
			      ...
			      if (debugRcdaqEventReadoutLock) {
				pthread_mutex_lock(&M_cout);
				cout << __FILE__ << " LINE " << __LINE__ << " set rcdaqdataready TRUE" << endl;
				pthread_mutex_unlock(&M_cout);
			   }
			      ...
			      if (debugRcdaqEventReadoutLock) {
				pthread_mutex_lock(&M_cout);
				cout << __FILE__ << " LINE " << __LINE__ << "  WAIT on DataReadoutDone for RCDAQ data read completion " << endl;
				pthread_mutex_unlock(&M_cout);
			      }
			      ...
			      if (debugRcdaqEventReadoutLock) {
				pthread_mutex_lock(&M_cout);
				cout << __FILE__ << " LINE " << __LINE__ << "  GOT LOCK on DataReadoutDone  " << endl;
				pthread_mutex_unlock(&M_cout);
			      }
			      rcdaqdataready = false;
			      if (debugRcdaqEventReadoutLock) {
				pthread_mutex_lock(&M_cout);
				cout << __FILE__ << " LINE " << __LINE__ << " set rcdaqdataready FALSE UNLOCKED DataReadyReset" << endl;
				pthread_mutex_unlock(&M_cout);
			      }
			      pthread_mutex_unlock( &DataReadyReset ); // enable trigger ready test
